...
// Neutral initial camera only. It is never labelled as the user or a clinic.
const DEFAULT_MAP_CENTER: LatLng = [41.2858, 69.2463];
...
const YandexCanvas = forwardRef<MapCanvasHandle, MapCanvasProps>(function YandexCanvas(
  ...
  const userRef = useRef<LatLng | null>(user);
  ...
  useImperativeHandle(
    ...
    () => ({
      recenter(target?: LatLng) {
        mapRef.current?.setCenter(target ?? userRef.current ?? DEFAULT_MAP_CENTER, 15, { duration: 300 });
      },
    ...
    loadYandex()
      .then((ymaps) => {
        ...
        const map = new ymaps.Map(
          nodeRef.current,
          { center: userRef.current ?? DEFAULT_MAP_CENTER, zoom: 13, controls: ["geolocationControl"] },
  ...
  useEffect(() => {
    ...
    const positions = [
      ...
      ...clinics.map(({ position }) => position)
    ];
    if (positions.length === 0) {
      map.setCenter(DEFAULT_MAP_CENTER, 12);
      return;
    }
    if (positions.length === 1) {
      map.setCenter(positions[0], 15);
      return;
    }
...
const LeafletCanvas = forwardRef<MapCanvasHandle, MapCanvasProps>(function LeafletCanvas(
  ...
  useImperativeHandle(
    ...
    () => ({
      recenter(target?: LatLng) {
        mapRef.current?.setView(target ?? userRef.current ?? DEFAULT_MAP_CENTER, 15, { animate: true });
      },
      async searchTo(query: string) {
        const map = mapRef.current;
        if (!map) {
  ...
  useEffect(() => {
    ...
    async function mountMap() {
      ...
      const map = L.map(nodeRef.current, {
        ...
        touchZoom: true,
        zoomControl: false
      }).setView(userRef.current ?? DEFAULT_MAP_CENTER, 13);
  ...
  useEffect(() => {
    ...
    if (positions.length === 0) {
      map.setView(DEFAULT_MAP_CENTER, 12, { animate: false });
      return;
    }
    if (positions.length === 1) {
...
export function MapView({
  ...
  const mapClinics = useMemo<MapClinicMarker[]>(
    () =>
      featuredClinics.map((clinic, index) => ({
        clinic,
        position: [clinic.lat as number, clinic.lng as number] as LatLng,
  ...
  const centerNearby = useCallback(async () => {
    ...
    if (coords) {
      ...
      setLocationError("Joylashuvingiz aniqlanmadi. Brauzer ruxsatini tekshiring.");
    }
  }, []);
  ...
  return (
    <section
      ...
      aria-label="Klinikalar xaritasi"
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-center gap-2.5 px-4 pt-4">
        ...
        >
          ...
          <input
            aria-label="Xaritada manzil yoki joyni qidirish"
            type="search"
            enterKeyHint="search"
            value={mapQuery}
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col items-end">
        <button
          ...
          disabled={locating}
          className="mb-3 mr-4 inline-flex items-center gap-2 rounded-pill bg-surface-0 px-4 py-2.5 text-sm font-semibold text-brand-600 shadow-float transition-colors hover:bg-brand-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 focus-visible:ring-offset-surface-0 active:scale-95 disabled:opacity-70"
        >
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            <span className="flex min-w-0 flex-col">
              ...
              <small className="text-xs text-ink-500">
                {featuredClinics.length} ta klinikaning aniq lokatsiyasi ko&apos;rinmoqda
              </small>
            </span>
            ...
            {featuredClinics
              ...
              .map((clinic) => (
                ...
                >
                  ...
                  <span className="inline-flex shrink-0 items-center gap-0.5 text-xs font-semibold text-warning">
                    <Star size={12} />
